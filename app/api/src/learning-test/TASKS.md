...
This introduces a _different_ mocking API — timers, not modules.
...
- [ ] Test `charge()` throws for `amountCents <= 0` — and confirm the SDK's `createCharge` was _never_ called in that case.
...
2. If your mock factory for `SearchSdk` references an outside `vi.fn()`, what has to be true about _when_ that `vi.fn()` was created?
