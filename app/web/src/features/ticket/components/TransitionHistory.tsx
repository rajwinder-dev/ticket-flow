    ...
    <Card className={className}>
      ...
      <CardContent>
        {isLoadingTicketTransitions ? (
            ...
            {Array.from({ length: 3 }).map((_, index) => (
              <div key={index} className="relative flex items-start gap-4">
                ...
                <div className="flex flex-col items-center">
                  ...
                  {index < 2 && <Skeleton className="bg-muted/40 mt-2 h-12 w-0.5" />}
                </div>
                <div className="flex-1 space-y-1.5 pt-0.5">
                  <div className="flex items-center justify-between gap-4">
                    <Skeleton className="h-4 w-32" /> {/* Event Action Name label placeholder */}
          ...
          <p className="text-muted-foreground py-6 text-center text-sm italic">No activity yet.</p>
        ) : (
