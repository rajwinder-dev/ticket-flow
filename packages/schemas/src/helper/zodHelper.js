import { z } from "zod";
// --- Base Validations ---
export const emptyToUndefined = (val) => typeof val === "string" && val.trim() === "" ? undefined : val;
// Fixed: z.email() is not a function, it's z.string().email()
export const optionalInput = (schema) => z.preprocess(emptyToUndefined, schema.optional());
...
export const validPermissions = z.record(z.string().trim().min(1), z.array(z.string().trim().min(1)).min(1, "Each resource must have at least one permission"));
