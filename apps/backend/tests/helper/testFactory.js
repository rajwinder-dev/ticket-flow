...
export class testFactory {
    ...
    async setup(username = "rajwinder", password = "user") {
    ...
    async logout(expectedStatus = 200) {
        ...
        if (!this.token || !this.cookie) {
            throw new Error("Cannot logout: token or cookie is missing. Did you forget to call setup()?");
        }
        const res = await request
            .agent(app)
            .post(logoutPath)
    ...
    async post({ path, data, expectedStatus = 201, id }) {
        ...
        try {
            if (this.logs)
                this.logOutput("POST", fullPath, data, res.body, true);
    ...
    async patch({ path, data, expectedStatus = 200, id }) {
        let fullPath = `${endpoint}${path}`;
        if (id) {
            fullPath += `/${id}`;
        }
        ...
        try {
            if (this.logs)
                this.logOutput("PATCH", fullPath, data, res.body, true);
            expect(res.statusCode).toBe(expectedStatus);
        }
        catch (err) {
    ...
    async get({ path, expectedStatus = 200, id }) {
        let fullPath = `${endpoint}${path}`;
        if (id) {
            fullPath += `/${id}`;
        }
        ...
        try {
            if (this.logs)
                this.logOutput("GET", fullPath, null, res.body, true);
            expect(res.statusCode).toBe(expectedStatus);
        }
        catch (err) {
    ...
    async delete({ path, expectedStatus = 204, id }) {
        let fullPath = `${endpoint}${path}`;
        if (id) {
            fullPath += `/${id}`;
        }
        ...
        try {
            if (this.logs)
                this.logOutput("DELETE", fullPath, null, res.body, true);
            expect(res.statusCode).toBe(expectedStatus);
        }
        catch (err) {
    ...
    logOutput(requestType, fullPath, input, output, success) {
        ...
        const outputString = output
            ...
            : "";
        console.log(infoString + inputString + outputString + `TimeStamp: ${new Date()}`);
    }
}
